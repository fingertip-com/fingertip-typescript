...
```sh
npm install git+ssh://git@github.com:stainless-sdks/fingertip-typescript.git
```
...
1. Via the `FINGERTIP_LOG` environment variable
...
const client = new Fingertip({
  logger: logger.child({ name: 'Fingertip' }),
...
const client = new Fingertip({ fetch });
```
...
```ts
import Fingertip from 'npm:fingertip';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/fingertip-typescript/issues) with questions, bugs, or suggestions.
